    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
          label="载重" v-if="this.vehicleShwo">
          <a-input placeholder="请输入载重" v-decorator.trim="['itemAttachValue', validatorRules.itemAttachValue]"/>
        </a-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          ...
          itemAttachValue: { rules: [{ required: true, message: '请输入车辆载重!' }] }
        },
        vehicleShwo: false
      }
    },
    ...
    methods: {
      add(dictId, val) {
        // 如果操作的是车辆类型就会显示载重输入框
        if (val == 'vehicle_type') {
          this.vehicleShwo = true // 载重输入框做展示
        }
        this.dictId = dictId
        this.edit({})
      },
      edit(record, dictId, val) {
        // 如果操作的是车辆类型就会显示载重输入框
        if (val == 'vehicle_type') {
          this.vehicleShwo = true // 载重输入框做展示
        }
        ...
        this.visible = true
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model, 'itemText', 'itemValue', 'itemAttachValue', 'description', 'sortOrder'))
        })
      },
      onChose(checked) {
        if (checked) {
