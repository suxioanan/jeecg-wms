  ...
  export default {
    ...
    methods: {
      ...
      wandanplanmodelShow(record) {
        ...
        that.accomplishNum=record.accomplishNum
        that.accomplishVolume=record.accomplishVolume
      ...
      requestTableData(url, params, tab) {
        ...
        getAction(url, params).then(res => {
          ...
          this.$nextTick(() => {
            ...
            if (this.operatorSchema == 0) {
              console.log("计算前 》》：", this.defaultTable.dataSource[0].numReceipts)
              this.defaultTable.dataSource[0].numReceipts = ((Number(this.defaultTable.dataSource[0].numReceipts) * 1000 + Number(this.wdrecord.accomplishNum) * 1000) / 1000)
              console.log("计算后 》》：", this.defaultTable.dataSource[0].numReceipts)
            }
            //时间格式化
      ...
      requestAddOrEdit(formData) {
        ...
        formData.ptype=1
        //异常照片
        var phont1=formData.completeOrderList[0].scenePhotos1
        var phont2=formData.completeOrderList[0].scenePhotos2
        if(phont1!=null){
          formData.completeOrderList[0].scenePhotos1=phont1.name
        }
        if(phont2!=null){
          formData.completeOrderList[0].scenePhotos1=phont2.name
        }
        if(phont1!=null&&phont2!=null){
          formData.completeOrderList[0].scenePhotos1=phont1.name+','+phont2.name
        }else {
          formData.completeOrderList[0].scenePhotos1=""
        }
        // 完单数量差值
