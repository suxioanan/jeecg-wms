        ...
        <a-row :gutter="24">
          <a-col :md="6" :sm="12">
            <a-form-item label="派单类型">
              <a-select v-decorator="['operatorSchema',validatorRules.operatorSchema]" placeholder="请选择派单类型" @change="changeOperatorSchema" :disabled="true">
          ...
          <a-col :md="6" :sm="12">
            <a-form-item label="员工">
              <j-multi-select-tag
                ...
                :options="dictOptions"
                dictCode="sys_user,realname,realname,status='1' and username!='admin'"
                placeholder="请做出你的选择">
              </j-multi-select-tag>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url:{
          add: '/cable/sendOrders/mergePlanEdit',
          edit: '/cable/sendOrders/mergePlanEdit',
    ...
    methods: {
      ...
      mergePlanModelEditShow(record) {
        this.record = record
        console.log("进入了派单编辑页面",record)
        this.activeKey = '1'

        this.ids = record.planId
        this.paln = record.planType
        let urlRu = '';
        let urlChu = '';
        if(this.paln == '1') {
        ...
        this.model.operatorSchema = this.record.operatorSchema + ''
        this.changeOperatorSchema(this.model.operatorSchema)
        this.model.backup3 = this.record.linkman
        this.model.backup4 = this.record.phone
        this.model.backup2 = this.record.address
        this.model.realname = this.record.a1
        this.endWarehouseId = this.record.endWarehouseId

        /* 派单车辆回显数据 begin */
        var list = new Array();
        for (let i = 0; i < this.record.a0.split(",").length; i++) {
          var map = {
            license:this.record.a0.split(",")[i],
            number:1
          }
          list.push(map)
        }
        this.table2.dataSource = list
        console.log("派单车辆回显数据 list >>>:",this.table2.dataSource)
        /* 派单车辆回显数据 end */

        // 加载子表数据
        if (this.ids) {
          let params = { ids: this.ids }
          if (this.model.operatorSchema == '1')
          this.requestTableData(urlRu, params, this.tableRu)
            if (this.model.operatorSchema == '0')
      ...
      requestTableData(url, params, tab) {
        ...
        getAction(url, params).then(res => {
          ...
          // 编辑为入库时
          if (this.model.operatorSchema == '1'){
            this.tableRu.dataSource[0].backup1 = this.record.backup1
          }
          // 编辑为出库时
          if (this.model.operatorSchema == '0'){
            this.tableChu.dataSource[0].backup1 = this.record.backup1
            this.tableChu.dataSource[0].endWarehouseId = this.record.endWarehouseName
            console.log("计算前 》》：",this.tableChu.dataSource[0].inventoryQuantity)
            this.tableChu.dataSource[0].inventoryQuantity = ((Number(this.tableChu.dataSource[0].inventoryQuantity) * 1000 + Number(this.record.backup1) * 1000) / 1000)
            console.log("计算后 》》：",this.tableChu.dataSource[0].inventoryQuantity)
          }
          console.log("派单信息回显数据 list >>>:",this.tableRu.dataSource)
          // 派单信息回显数据 end -------------

          // 派单编辑公用数据回显
          this.$nextTick(() => {
            this.form.setFieldsValue(pick(this.model, 'operatorSchema', 'taskTime', 'realname', 'backup2', 'backup3', 'backup4'))
            //时间格式化
            this.form.setFieldsValue({ orderDate: this.model.taskTime ? moment(this.model.taskTime) : null })
          })
          tab.loading = false
        })
      },
      ...
      requestAddOrEdit(formData) {
        ...
        //如果下拉框的值类型是 ‘String’ ，终点仓库 取 初始的仓库id，否则取当前仓库id
        if(formData.jeecgOrderCustomerList[0].endWarehouseId != undefined)
          formData.jeecgOrderCustomerList[0].endWarehouseId = (isNaN(formData.jeecgOrderCustomerList[0].endWarehouseId) ? this.endWarehouseId : formData.jeecgOrderCustomerList[0].endWarehouseId)
